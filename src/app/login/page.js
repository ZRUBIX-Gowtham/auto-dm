...
import { logInWithEmail, logInWithGoogle, getLiveCurrentUser } from "@/firebase/auth";
      ...
      <div className="card auth-card animate-pulse-glow">
        <div className="auth-header">
          ...
          <p style={{ color: "var(--text-secondary)", marginTop: "0.5rem" }}>
            Log in to manage your messaging automations
          </p>
        </div>
        ...
        {isSimulationMode && (
          <div className="badge badge-primary" style={{ width: "100%", padding: "0.75rem", textTransform: "none", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "0.25rem", marginBottom: "1.5rem", borderRadius: "var(--radius-sm)", fontWeight: 500, lineHeight: 1.4 }}>
            ...
            <span style={{ fontSize: "0.8rem", opacity: 0.9 }}>Firebase not configured. Type ANY email and password to log in or create an account locally.</span>
          </div>
        )}
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" className="btn btn-primary" style={{ width: "100%", height: "48px" }} disabled={loading}>
            {loading ? <div className="spinner" style={{ borderTopColor: "white", width: "20px", height: "20px" }}></div> : "Sign In"}
          </button>
        </form>
